...
# 当前演示版使用内存保存会话。
# Render重启或重新部署后，这些数据会被清空。
SESSION_STORE: Dict[str, Dict[str, Any]] = {}


# 当前是测试案例库。
# 正式版本需要替换成真实往届获奖作品数据库。
REFERENCE_CASES = [
    {
...
def score_candidate(
    ...
    application_score = (
        ...
        + min(
            12,
            len(fields["target_groups"]) * 4
        )
        + min(
            8,
            len(fields["scenarios"]) * 3
        )
    )
    ...
    data_completeness_bonus = (
        completeness_items * 1.5
    )

    differentiation_bonus = 0

    combined_modules = "".join(
        functions + tech_modules
    )
    ...
    return {
        "innovation_score": round(
            clamp(innovation_score),
            1
        ),
        "scientific_score": round(
            clamp(scientific_score),
            1
        ),
        "application_score": round(
            clamp(application_score),
            1
        ),
        "expression_score": round(
            clamp(expression_score),
            1
        ),
        ...
        "homogeneity_penalty": round(
            homogeneity_penalty,
            1
        ),
        "feasibility_penalty": round(
            feasibility_penalty,
            1
        ),
        "data_completeness_bonus": round(
            data_completeness_bonus,
            1
        ),
        "differentiation_bonus": round(
            differentiation_bonus,
            1
        ),
        "competitiveness_score": round(
            clamp(competitiveness_score),
            1
        )
    }
...
def generate_candidates(
    ...
    for index, candidate in enumerate(candidates):
        candidate["scores"] = score_candidate(
            fields,
            index
        )
...
def format_candidate_message(
    ...
    lines = [
        ...
        (
            "目标用户："
            + "、".join(fields["target_groups"])
        ),
        (
            "应用场景："
            + "、".join(fields["scenarios"])
        ),
        (
            "核心功能："
            + "、".join(fields["core_functions"])
        ),
        (
            "技术模块："
            + "、".join(fields["tech_modules"])
        ),
        "",
        "【3个候选题目与获奖竞争力预测】"
    ]
    ...
    for candidate in candidates:
        ...
        lines.extend([
            ...
            (
                ...
                f"最高相似度："
                f"{scores['highest_similarity']}；"
                f"相似案例："
                f"{scores['similar_case']['title']}"
            ),
            (
                "惩罚/加分："
                f"相似度惩罚-"
                f"{scores['similarity_penalty']}，"
                f"同质化惩罚-"
                f"{scores['homogeneity_penalty']}，"
                f"可实现性惩罚-"
                f"{scores['feasibility_penalty']}，"
                f"资料完整度+"
                f"{scores['data_completeness_bonus']}，"
                f"差异化创新+"
                f"{scores['differentiation_bonus']}"
            ),
            (
                "获奖竞争力预测："
                f"{scores['competitiveness_score']} / 100"
            )
        ])
...
def build_report_json(
    ...
    """
    生成严格固定为七页的结构化报告JSON。
    第2页至第7页分别预留一张配图，共6张配图。
    """
    title = selected["title"]
    scores = selected["scores"]

    target = "、".join(
        fields.get("target_groups", [])
    )
    ...
    core_functions = fields.get(
        "core_functions",
        []
    )

    tech_modules = fields.get(
        "tech_modules",
        []
    )
    ...
    report = {
        "schema_version": (
            "robot-competition-report-v0.3.1"
        ),
        "project_title": title,
        "revision": 1,
        ...
        "competition_prediction": {
            ...
            "base_total": scores.get(
                "base_total",
                0
            ),
            "innovation_score": scores.get(
                "innovation_score",
                0
            ),
            "scientific_score": scores.get(
                "scientific_score",
                0
            ),
            "application_score": scores.get(
                "application_score",
                0
            ),
            "expression_score": scores.get(
                "expression_score",
                0
            ),
            "highest_similarity": scores.get(
                "highest_similarity",
                0
            ),
            "similar_case": scores.get(
                "similar_case",
                {}
            ),
            "similarity_penalty": scores.get(
                "similarity_penalty",
                0
            ),
            "homogeneity_penalty": scores.get(
                "homogeneity_penalty",
                0
            ),
            "feasibility_penalty": scores.get(
                "feasibility_penalty",
                0
            ),
            "data_completeness_bonus": scores.get(
                "data_completeness_bonus",
                0
            ),
            "differentiation_bonus": scores.get(
                "differentiation_bonus",
                0
            ),
            "disclaimer": (
        ...
        "project_fields": {
            "target_groups": fields.get(
                "target_groups",
                []
            ),
            "scenarios": fields.get(
                "scenarios",
                []
            ),
        ...
        "pages": {
            ...
            "page_7": {
                ...
                "title": "行业应用前景",
                "content": {
                    "application_scenarios": fields.get(
                        "scenarios",
                        []
                    ),
                    "target_users": fields.get(
                        "target_groups",
                        []
                    ),
                    "deployment_paths": [
...
def handle_chat(
    ...
    """
    session_key = (
        "chaoxing-robot-competition-demo"
    )
    ...
    if intent == "select_title":
        ...
        message = (
            ...
            f"{selected['title']}\n\n"
            "该题目的获奖竞争力预测为："
            f"{selected['scores']['competitiveness_score']}"
            " / 100。\n\n"
            "下一步请输入“生成报告”，"
            "系统将生成严格固定为7页的"
            "结构化报告JSON。"
...
def health() -> Dict[str, str]:
    return {
        "status": "ok",
        "service": (
            "robot_competition_orchestrator"
        ),
        "version": "0.3.1"
    }
